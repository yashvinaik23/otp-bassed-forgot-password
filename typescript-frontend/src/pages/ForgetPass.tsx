...
const ForgetPass = () => {
  ...
  const {
    ...
    correctOtp,
  } = useSelector((state: RootState) => state.forgotPasswordStore);
  ...
  const [openConfirmPassword, setOpenConfirmPassword] = useState(
    !!otpSet && otpSet?.sendotp && !!correctOtp ? true : false
  );
  ...
  useEffect(() => {
    if (counter > 0) {
      const timer = setInterval((): void => setCounter(counter - 1), 1000);
  ...
  }, [resetPasswordSuccess, isSendingOtp, correctOtp, dispatch, otpByPhone]);
  ...
  const handleResetPasswordConfirm = (event: MouseEvent<HTMLElement>) => {
    event?.preventDefault();
    const email = localStorage.getItem("resetPassEmail");
    const contact = localStorage.getItem("contact");
    if (
  ...
  const onChangeTextPass = (event: React.ChangeEvent<HTMLInputElement>) => {
